...
3. Crie um arquivo chamado `local.properties` na raiz do projeto e adicione suas credenciais da API da Pluggy (obtidas no dashboard [meu.pluggy.ai](https://meu.pluggy.ai)):
   ```properties
   PLUGGY_CLIENT_ID=seu_client_id_aqui
   PLUGGY_CLIENT_SECRET=seu_client_secret_aqui
   PLUGGY_ACCOUNT_ID=seu_account_id_aqui
   ```
4. Aguarde o *Gradle Sync* finalizar para baixar todas as dependências (Koin, Retrofit, Room, etc.).
